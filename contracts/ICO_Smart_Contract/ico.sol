...
contract SageICO is SageToken{
    ...
    uint tokenPrice = 0.001 ether; // 1 ETH = 1000 SAGETOKEN, 1SAGETOKEN = 0.001ETH
    ...
    receive() payable external{
     ...
     // locking of funds implementation

    function transfer(address to, uint tokens) public  override returns (bool success){
        require(block.timestamp > tokenTradeStart);
        SageToken.transfer(to, tokens); // same as super.transfer(to, tokens);
        return true;
    }   

    function transferFrom(address from, address to, uint tokens) public override returns (bool success){
        require(block.timestamp > tokenTradeStart);
        SageToken.transferFrom(from, to, tokens);
        return true;
    }
